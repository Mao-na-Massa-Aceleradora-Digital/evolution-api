...
export abstract class RouterBroker {
  ...
  public async dataValidate<T>(args: DataValidate<T>) {
    ...
    if (request?.query && Object.keys(request.query).length > 0) {
      // fetchInstances não tem ":instanceName" na URL; instanceId/instanceName legitimamente vêm da query string.
    ...
    if (request.originalUrl.includes('/instance/create')) {
      // instanceName legitimamente vem do body aqui (não há :instanceName na URL); só instanceId é gerado pelo servidor.
  ...
  public async groupValidate<T>(args: DataValidate<T>) {
    ...
    const instance = request.params as unknown as InstanceDto;
    // GET sem "Content-Type: application/json" chega com request.body undefined (express.json só popula em requests JSON).
    const body = request.body ?? {};
